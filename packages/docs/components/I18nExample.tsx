  ...
  return (
    <section class="i18n-example" aria-label="Interactive Kamod i18n example">
      <div class="i18n-example__toolbar">
        <span class="i18n-example__status">
          {loading ? "Loading locale…" : `locale: ${locale}`}
        </span>
      ...
      <div class="i18n-example__facts">
        ...
        <div>
          <span>{t("price")}</span>
          <strong>{number(128.4, { style: "currency", currency: "CHF" })}</strong>
        </div>
        <div>
          <span>Release date</span>
          <strong>
            {date(new Date("2026-08-24T12:00:00Z"), { dateStyle: "long", timeZone: "UTC" })}
          </strong>
        </div>
      </div>
    </section>
