...
import type { I18n, InterpolationValues, Listener, LocaleSource, MessageKey, MessageTree, MissingKeyInfo, PluralMessage, Unsubscribe } from './types'

const isLocaleLoader = <TMessages>(value: TMessages | (() => unknown)): value is () => unknown => typeof value === 'function'
...
export const createI18n = <const TSources extends Record<string, LocaleSource<MessageTree>>>(
  ...
  const relativeTimeFormatter = createFormatterCache<Intl.RelativeTimeFormatOptions, Intl.RelativeTimeFormat>(
  ...
  const hasLocale = (locale: string): locale is LocaleKey<TSources> => Object.prototype.hasOwnProperty.call(sources, locale)
  ...
  const t = (key: MessageKey<SourceSchema<TSources[keyof TSources]>>, values: InterpolationValues = {}): string => {
    ...
    const category = typeof count === 'number' || typeof count === 'bigint'
