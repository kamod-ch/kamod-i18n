...
`@kamod/i18n` is deliberately small: a typed translation lookup, tiny interpolation, plural selection through `Intl.PluralRules`, and formatting through native JavaScript `Intl` APIs.
...
```bash
pnpm add @kamod/i18n
pnpm add preact # only when using @kamod/i18n/preact
```
